...
```{r}
#| echo: true
table(predict(try1), gator1$choice, dnn = c("predicted", "actual"))
```
...
table(predict(fit_LS), gator2$food, dnn = c("predicted", "actual"))
```
...
table(predict(fit_SAT), gator2$food, dnn = c("predicted", "actual"))
```
...
table(predict(fit_1), gator2$food, dnn = c("predicted", "actual"))
```

- `fit_SAT` only predicts "fish" and makes 94 correct predictions out of 219 opportunities (42.9% correct.)
